...
2. DiskState 写入只有两条入口：已打开文件由用户 Cmd+S 保存写入；未打开文件由 accepted 路径在 baseRevision 校验通过后写入。
...
6. 已打开文件的 diff 创建时，LogicalState 立即在 originalText 位置精确替换为 newText，diff 进入 preapplied；Editor 展示绿增效果（只显示新增内容绿色覆盖，不显示红删）；用户接受时仅移除绿增效果（LogicalState 不变，DiskState 不写入，文件保持 dirty 直到 Cmd+S 保存）；用户拒绝时 LogicalState 回滚到 originalText。
...
| 名词 | 定义 |
|------|------|
| **PendingDiff** | AI 内容编辑请求生成的变更提案对象，包含 originalText、newText、sourceToolId、filePath、baseRevision 等字段；在被接受或拒绝前处于可审阅状态 |
...
| **preapplied** | LogicalState 已在 originalText 位置精确替换为 newText；Editor 展示绿增效果；等待用户接受或拒绝 |
| **accepting / rejecting** | 用户触发接受或拒绝操作后的短暂内存态（不写入 DB）；crash 发生时重启后按 preapplied 或 pending 重新处理 |
| **accepted（已打开文件）** | 绿增效果消除；LogicalState 不变（已含 newText）；DiskState 未修改；文件保持 dirty 直到用户 Cmd+S 保存 |
| **accepted（未打开文件）** | DiskState 在 originalText 匹配位置精确替换为 newText；文件内容更新 |
...
| REQ-DE-002 | 接受 diff | 用户接受 PendingDiff 后，diff 进入 accepted 终态。已打开文件：绿增效果消除，LogicalState 不变（已含 newText），DiskState 不写入，文件保持 dirty。未打开文件：DiskState 在 originalText 匹配位置精确替换为 newText。 | 已打开文件：接受后绿增消除，编辑器内容已含 newText（因 diff 创建时 LogicalState 已修改），文件保持 dirty 直到 Cmd+S 保存；DiskState 在接受时不触碰。未打开文件：接受后磁盘对应片段与 newText 一致。两种路径 PendingDiff 均不再可执行。 |
...
| REQ-DE-012 | 未打开文件 diff 继承流 | 当 update_file 工具对未打开文件生成 pending diff 后，用户在 Editor 中打开该文件时，系统须自动检验 DiskState 与 baseRevision 是否一致：一致则在 LogicalState 中将 originalText 精确替换为 newText，diff 升级为 preapplied 状态（effectivePath 升级为 open-file），Editor 展示绿增；不一致则 diff 自动转 expired，不继承。 | 文件被打开后：DiskState hash 一致时绿增效果自动出现，diff 变为 preapplied；不一致时 diff 卡片显示 expired；继承后 accept 不写入 DiskState（走 open-file 路径）；继承失败（hash 不一致）不显示绿增，不抛错，diff 自然进入 expired。 |
| REQ-DE-013 | 批量操作栏 | 聊天面板中，当存在数量 ≥ 1 的非终态（pending / preapplied）PendingDiff 时，输入框上方必须展示 DiffActionBar，提供"接受全部 (N)"和"拒绝全部 (N)"两个操作入口；全部进入终态后自动隐藏。批量操作每条独立走状态机，失败项标记 error 终态，不阻止其他条继续执行。 | 有 pending/preapplied diff 时 DiffActionBar 可见且按钮可点；批量操作后每条 diff 独立进入 accepted / rejected 终态；全部进入终态后 DiffActionBar 自动消失；无 pending/preapplied diff 时不渲染 DiffActionBar。 |
...
|---------|-------|-------|--------|--------|-----------|-----------|
| S01 | AG | edit_current_editor_document 工具调用（originalText、newText、summary、sourceToolId）| 路由到 DE 创建 diff | 工具参数 | S02 | — |
...
| S04 | DE | originalText + newText + sourceToolId + filePath | 创建 PendingDiff（status=pending，含 sourceToolId、baseRevision、createdAt）| PendingDiff | S05 | ERR-03（创建失败）|
...
| step_id | actor | input | action | output | next_step | exception |
|---------|-------|-------|--------|--------|-----------|-----------|
| S01 | AG | update_file 工具调用（filePath、originalText、newText、summary、sourceToolId）| 路由到 DE 创建 diff | 工具参数 | S02 | — |
...
| S04 | ED | diffId | 移除对应绿增 Decoration（LogicalState 不变，已含 newText）| 绿增效果消除 | S05 | ERR-02（Decoration 移除失败 → 继续）|
...
| S03 | SYS | filePath | 读取目标文件 DiskState，计算 hash | 当前 hash | S04 | ERR-02（文件读取失败）|
...
| S06 | SYS | originalText + newText + filePath | 在 DiskState 中精确替换 originalText 为 newText | 写入结果 | S07（成功）| ERR-04（写入失败 → diffMachine → error）|
...
持久化与恢复：PendingDiff 写入 workspace.db 的 pending_diffs 表；Workspace 重新打开时加载 status 为 pending 的记录，计算目标文件 DiskState hash 并与记录 baseRevision 比对，不一致时自动转 expired；Workspace 关闭时所有非终态 diff 转 expired 并写入 DB。
...
| S04a | DE | 该文件全部 pending diff | 批量执行 ACCEPT（在 originalText 位置写入 newText，diff → accepted）| 文件内容更新 | S05 | 失败项 → error 终态，其余继续 |
...
| S05 | DE | 每条 pending 记录 | 计算目标文件 DiskState hash；与记录 baseRevision 比对 | 校验结果 | S06（一致 → 恢复为 pending）/ S07（不一致 → 转 expired）| — |
...
| step_id | actor | input | action | output | next_step | exception |
|---------|-------|-------|--------|--------|-----------|-----------|
| S01 | AG | 新内容编辑工具调用（filePath + originalText + newText）| 路由到 DE 创建 diff | 工具参数 | S02 | — |
| S02 | DE | 新 PendingDiff 参数 | 正常执行 DE-CREATE-DIFF 流程（已打开文件）| 新 PendingDiff | S03 | — |
| S03 | DE/ED | newText | 在旧 preapplied 区域对应 LogicalState 中执行精确替换 | LogicalState 更新 | S04 | — |
...
| 类别 | 决策 |
|------|------|
| **Cmd+S 时有 preapplied diff 的确认流程** | 必须弹窗确认。用户在编辑器执行 Cmd+S 时，若 ActiveFile 有 preapplied diff，必须弹出确认框"保存将固化 ActiveFile 全部 AI 修改"；用户确认后对该文件所有 preapplied diff 执行 ACCEPT（移除绿增效果），再将 LogicalState 写入 DiskState（Cmd+S 保存）；用户取消则终止保存；不影响其他文件的 diff。DiskState 只在 Cmd+S 写盘时修改，accept 动作本身不写盘。对齐 binder-core BR-DE-DIFF-005。 |
...
| 方向 | 触发场景 | 数据边界 | 约束 |
|------|----------|----------|------|
| DE ← AG | 内容编辑工具调用 | AG工具参数（filePath、originalText、newText、sourceToolId）→ DE 创建 PendingDiff | AG 不得直接写文件；所有内容修改必须经 DE 路由 |
...
| DE → ED | preapplied 状态下用户接受 diff | diffId → ED 移除绿增 Decoration | Accept 不推送内容（LogicalState 已含 newText）；不写磁盘；仅移除视觉效果 |
| DE → WS | 未打开文件 Diff accept 时写入目标文件 | originalText + newText + filePath → SYS 文件写入 | 写入前必须通过 WS 边界校验（文件在 workspaceRoot 内）并通过 baseRevision 校验 |
...
| 2026-05-26 | v1.7 | §3 新增 REQ-DE-013（批量操作栏 DiffActionBar：输入框上方；pending/preapplied 数 ≥ 1 时可见；全部终态后自动隐藏）；§7 新增 REQ-DE-013 标注块 |
