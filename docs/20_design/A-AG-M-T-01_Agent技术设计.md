...
直接承接：   Phase 10-12 Issue Trace、agentMachine、agentService 实现
...
stateDiagram-v2
    [*] --> idle
    idle --> validatingProvider : SEND_REQUESTED
...
| 状态 | 含义 | 约束 |
|------|------|------|
| idle | 等待用户输入 | 可接收 SEND_REQUESTED |
| validatingProvider | 校验 Provider 配置 | Provider 无效时转 error |
| sending | 已提交 Provider 请求 | 等待流式开始信号 |
| streaming | 接收流式响应 token | 可被 tool_call 打断 |
| toolCalling | 执行工具调用 | 工具完成后返回 streaming |
| error | 出错等待恢复 | 下一次 SEND_REQUESTED 重试 |

### 2.2 扩展要求（Phase 10）

Phase 10 实现时 agentMachine 需要扩展：

1. 增加 `cancelling` 状态：用户主动取消时进入，完成清理后转 idle。
2. error 状态必须携带错误类型（provider_error / network_error / tool_error / timeout）。
3. streaming 不得在工具调用完成前宣告 RESPONSE_FINISHED。
...
### 3.5 InputReference（已有）

```ts
interface InputReference {
  id: string;
  target: WorkspaceFileTarget;
  mode: "readonly";   // 必须强制 readonly，不得修改
}
```
...
| create_file | 直接创建（无内容写入） | PathConflict 检查 |
...
2. `allowedTools` 由 PromptRuntime 指定，不由模型自行扩展。
3. InputReference 内容只注入 system 或 context 层，不替代 user message。
4. 历史裁剪时不得把工具结果注入成 user message。
5. 禁止字段（API key、内部路径、用户身份）必须在后端组装时被过滤。
...
3. API key 不出现在前端代码、日志或 Redux/Store 中。
4. agentMachine 状态转移覆盖 cancelling 路径。
...
1. allowedTools 过滤生效，模型不能调用未授权工具。
