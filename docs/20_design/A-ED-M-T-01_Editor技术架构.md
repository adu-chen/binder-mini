...
直接承接：   后续 Editor BlockId、DiffDecoration 实现 Issue Trace
...
TipTap/Markdown 选型以 `ED-M-T-02` 为准（已确认）。
...
初始建议：采用 `workspace.db` 映射表方案，不直接污染 Markdown 源文；实现前必须补专项设计。
...
1. 数据来源必须是 DE / Diff Review 产生的 PendingDiff。
